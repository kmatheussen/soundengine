...
class MyDsp extends Dsp {
    ...
    */
    ...
    public int getNumInputs()  { return 2; }
    public int getNumOutputs()     { return 2; }
    ...
    virtual void buildUserInterface(UI* interface) {
        ...
        interface->addHorizontalSlider("Damp", &fslider0, 0.5f, 0.0f, 1.0f, 2.
        500000e-02f);
        interface->addHorizontalSlider("RoomSize", &fslider1, 0.5f, 0.0f, 1.0f,
        2.500000e-02f);
        interface->addHorizontalSlider("Wet", &fslider2, 0.3333f, 0.0f, 1.0f, 2.
        500000e-02f);
        interface->closeBox();
    }
    */
...
public class FreeVerb extends MyDsp{
	...
	fslider2 = wet.get();
	super.compute(count,input,output);
    }
	...
	this.wet.set(wet);
	System.out.println("New wet val: "+wet);
    }
    FreeVerb reset(){
	float wet=this.wet.get_goal();
	float damp=this.damp.get_goal();
	float roomSize=this.roomSize.get_goal();
	FreeVerb freeVerb = new FreeVerb(fSamplingFreq);
	freeVerb.setRoomSize(roomSize);
	freeVerb.setDamp(damp);
	freeVerb.setWet(wet);
	return freeVerb;
    }
    public FreeVerb(int samplingFreq){ // Actually, samplingFreq is not really used for anything...
	init(samplingFreq);
